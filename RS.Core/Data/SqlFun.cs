...
select * from (
    select row_number() over({0}) as RecordNumberNo,totalAbyPage.* from ({1}) totalAbyPage
        ...
        public string GetPageSQL(string Sql, PageInfo page, IDbContext db, List<OrderItem> Orders = null)
        {
            string SelectCommand = Sql;
            if (Orders == null) Orders = new List<OrderItem>();
            if (page != null && page.PageSize > 0)
...
select * from (
    select row_number() over({0}) as RecordNumberNo,totalAbyPage.* from ({1}) totalAbyPage
        ...
        /// 设置合计行
        /// </summary>
        internal DynamicObj GetFootObj(PageInfo page,string with,string SelectCommand, IDbContext db)
        {
