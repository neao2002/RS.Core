...
namespace RS.Data
{
    ...
    internal class OracleContext : IDbContext
    {
        ...
        #region 构造函数
        public OracleContext(string strConnect)
        {
            m_conn = new OracleConnection(strConnect);
            connString = strConnect;
        }
        public OracleContext(OracleConnection cnn)
        {
            m_conn = cnn;
            connString = cnn.ConnectionString;
        }
        public OracleContext(OracleConnection cnn, OracleTransaction mytran)
        {
            m_conn = cnn;
            connString = cnn.ConnectionString;
            trans = mytran;
...
select * from (
    select row_number() over({0}) as RecordNumberNo,totalAbyPage.* from ({1}) totalAbyPage
        ...
        #region 删除记录
        public bool DeleteObject(string TableName,string KeyPropertyName,string KeyPropertyValue)
        {
            return DeleteObject(TableName, DynamicObj.NewObj(KeyPropertyName, KeyPropertyValue));
        }
        public bool DeleteObject(string TableName,DynamicObj KeyPropertyValues)
        {
            //未设置要更新的字段，则视为全部更新
            if (TableName.IsWhiteSpace())//未设置表名
            {
                throw new Exception("未设置要删除的对象表名");
            }
            if (KeyPropertyValues==null || KeyPropertyValues.Count==0)
            {
                throw new Exception("删除对象记录时未获取任何主键条件，为数据安全，这里不再进行任何保存。");
            }
            IFilter filter = LibHelper.CreateIFilter(this);
            foreach(string key in KeyPropertyValues.Keys)
            {
                object v = DBHelper.ConvertValue(KeyPropertyValues[key]);
                filter.AppendDefineItems(LibHelper.CreateSearchItem(Function.ConvertField(key), v.GetType().ToFilterDataType(), v));
            }
            if (filter.ListFilter.FilterItems.Count == 0)
            {
                throw new Exception("保存对象实例时未获取任何主键条件，为数据安全，这里不再进行任何保存。");
            ...
            cmd.CommandText = $"delete from {TableName} {filter.GetFilter("where")}";
